...
describe("Comments: data attribute wiring", () => {
  ...
  it("omits data-theme-url when no themeUrl is configured", () => {
    // the inline script then falls back to the official giscus theme base
    const result = renderComments(baseOpts, buildProps({ cfg: { baseUrl: "example.test" } }));
    const props = getDivProps(result);
    expect(props?.["data-theme-url"]).toBeUndefined();
  });
