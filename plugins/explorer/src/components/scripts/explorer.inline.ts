...
function renderTree(node, container, currentSlug, folderBehavior, savedState, pathPrefix = "") {
  ...
  if (node.isFolder) {
    ...
    // Default expanded: folders without a saved choice render open, and the
    // chevron collapses them (the sidebar always shows the full tree otherwise)
    const isCollapsed = savedState[node.slug] !== undefined ? savedState[node.slug] : false;
...
async function handleNavOrRender(e) {
  ...
  try {
    ...
    for (const explorer of allExplorers) {
      ...
      // Bind only when this run owns the current render. The nav AND render
      // events both funnel into this handler, so binding on every run would
      // attach every listener twice after an SPA navigation — one chevron
      // click then toggles twice, i.e. not at all.
      const shouldBind = thisGeneration === currentRenderGeneration;
      ...
      // Clicking anywhere on a folder row (not just its title link) follows the
      // folder link, matching how file rows navigate. The chevron stops
      // propagation for collapse, and clicks on the link itself are ignored so
      // the row handler never double-fires the navigation.
      const folderRows = explorer.getElementsByClassName("folder-container");
      if (!shouldBind) continue;
      for (const row of folderRows) {
        const rowClickHandler = function (evt) {
          if (explorer.dataset.behavior !== "link") return;
          if (evt.target.closest(".folder-icon") || evt.target.closest("a")) return;
          const link = row.querySelector("a.folder-button");
          if (!link) return;
          // synthetic <a> click bubbles to the window-level SPA router
          link.click();
        };
        row.addEventListener("click", rowClickHandler);
        cleanupHandlers.push(() => row.removeEventListener("click", rowClickHandler));
      }
