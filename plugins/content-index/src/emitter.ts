...
interface Options {
  ...
  /**
   * Slug prefixes excluded from the RSS feed (they still appear in the sitemap).
   * Defaults to tag pages and the 404 page, which otherwise carry no real date
   * and crowd out actual articles.
   */
  rssExcludeSlugPrefixes: string[];
}
...
function generateRSSFeed(
  ...
  const lastFewNotesText =
    options.rssLastFewNotesText ??
    ((count: number) => loc.components.rss.lastFewNotes({ count }));
  ...
  const items = Array.from(idx)
    .filter(([slug]) => {
      const simplified = simplifySlug(slug) as string;
      return !exclude.some(
  ...
  const description = locale.toLowerCase().startsWith("zh")
    ? `${summary} · ${escapeHTML(pageTitle)}`
    : `${summary} on ${escapeHTML(pageTitle)}`;
